...
export TMPDIR=/tmp/job_node2  # Force ALL temp files to local storage
export TRITON_CACHE_DIR=/tmp/triton_node2
export VLLM_CACHE_DIR=/tmp/vllm_cache_node2
export TORCH_COMPILE_CACHE_DIR=/tmp/torch_compile_cache_node2
export TORCHINDUCTOR_CACHE_DIR=/tmp/torch_compile_cache_node2  # Another torch.compile variable
export WANDB_DIR=/tmp/wandb_node2  # WandB directory to avoid NFS conflicts

# Override XDG_CACHE_HOME to force vLLM to use local cache
export XDG_CACHE_HOME=/tmp/xdg_cache_node2
...
export HF_HOME=/tmp/huggingface_node2
...
# vLLM isolation
export VLLM_INSTANCE_ID=node2
export VLLM_RAY_SESSION_DIR=/tmp/vllm_node2
export VLLM_WORKER_MULTIPROC_METHOD=spawn

echo "=========================================="
echo "Launching Node 2 Training Job"
echo "MASTER_ADDR: $MASTER_ADDR"
echo "MASTER_PORT: $MASTER_PORT"
echo "Cache dirs: $TRITON_CACHE_DIR"
echo "Log file: node2_training.log"
echo "=========================================="
...
accelerate launch \
  ...
  models/grpo.py \
  --output_dir models/trained/grpo_global_sentence_no_human_like \
  --use_semantic_similarity \
  --use_fluency \
  --resume_from_checkpoint models/trained/grpo_global_sentence_no_human_like \
  --wandb_run_id l11lnz86 \
  2>&1 | tee node2_training.log
